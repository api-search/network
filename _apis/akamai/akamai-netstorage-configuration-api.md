---
aid: akamai:akamai-netstorage-configuration-api
name: Akamai NetStorage Configuration API
tags: []
humanURL: https://techdocs.akamai.com/netstorage/reference/api
properties:
  - url: https://techdocs.akamai.com/netstorage/reference/api
    type: Documentation
description: >-
  NetStorage is a managed service that provides persistent, replicated storage
  of website content, including images, streaming media files, software,
  documents, and other digital objects. Content replicates periodically to core
  network locations to make it highly available to, and easily accessible by,
  EdgePlatform servers. NetStorage complements multiple content delivery
  services.

---