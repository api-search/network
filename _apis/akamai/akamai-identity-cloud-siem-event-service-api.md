---
aid: akamai:akamai-identity-cloud-siem-event-service-api
name: Akamai Identity Cloud SIEM Event Service API
tags: []
humanURL: https://techdocs.akamai.com/identity-cloud-siem-delivery/reference/api
properties: []
description: >-
  Security Event and Information Management (SIEM) is a recognized standard for
  collecting, aggregating, and analyzing events that take place on a website or
  within an app. Identity Clouds SIEM event delivery service can inform you, in
  near real-time, each time a specified event occurs. See Identity Cloud SIEM
  events for more information on the events available to the SIEM event service.

---