---
aid: mastercard:mastercard-cross-border-services
name: Mastercard Cross-Border Services
tags:
  - Borders
  - Payments
  - Global
  - International
humanURL: https://developer.mastercard.com/cross-border-services/documentation/
properties:
  - url: >-
      https://developer.mastercard.com/cross-border-services/documentation/api-basics/api-security/#authentication
    name: API Security | Mastercard Cross-Border Services | Mastercard Developers
    type: Authentication
  - url: >-
      https://developer.mastercard.com/cross-border-services/documentation/api-basics/api-security/#payload-encryption
    name: API Security | Mastercard Cross-Border Services | Mastercard Developers
    type: ' Encryption'
  - url: >-
      https://developer.mastercard.com/cross-border-services/documentation/api-basics/environments/
    name: Environments | Mastercard Cross-Border Services | Mastercard Developers
    type: Environments
  - url: >-
      https://developer.mastercard.com/cross-border-services/documentation/use-cases/
    name: Use Cases | Mastercard Cross-Border Services | Mastercard Developers
    type: UseCases
  - url: >-
      https://developer.mastercard.com/cross-border-services/documentation/tutorials/
    name: >-
      Tutorials and Guides | Mastercard Cross-Border Services | Mastercard
      Developers
    type: Tutorials
  - url: >-
      https://developer.mastercard.com/cross-border-services/documentation/api-ref/
    name: API Reference | Mastercard Cross-Border Services | Mastercard Developers
    type: Documentation
  - url: >-
      https://developer.mastercard.com/cross-border-services/documentation/ref-app/
    name: >-
      Reference Application | Mastercard Cross-Border Services | Mastercard
      Developers
    type: ReferenceApplication
  - url: >-
      https://developer.mastercard.com/cross-border-services/documentation/response-error-codes/
    name: >-
      Response Codes and Error codes | Mastercard Cross-Border Services |
      Mastercard Developers
    type: Errors
  - url: >-
      https://developer.mastercard.com/cross-border-services/documentation/release-announce/
    name: >-
      Bulletins and Release Announcements | Mastercard Cross-Border Services |
      Mastercard Developers
    type: Announcements
  - url: >-
      https://developer.mastercard.com/cross-border-services/documentation/support/
    name: Support | Mastercard Cross-Border Services | Mastercard Developers
    type: Support
description: >-
  Cross-Border Services is a global payment service that provides access to
  faster, transparent payments between Senders and Recipients through multiple
  channels and payout methods. Cross-Border Services provides:

---