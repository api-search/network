---
aid: amazon-web-services:pca-connector-ad
name: AWS Private CA Connector for Active Directory
tags:
  - ARN
  - Access
  - Connectors
  - Controls
  - Directory
  - Entries
  - Entry
  - Groups
  - Names
  - Principals
  - Registrations
  - Resources
  - Security
  - Services
  - Tags
  - Templates
  - Untag
image: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg
baseURL: https://example.com
humanURL: https://docs.aws.amazon.com/privateca/latest/userguide/connector-for-ad.html
overlays:
  - url: overlays/pca-connector-ad-openapi-search.yml
    type: APIs.io Search
  - url: overlays/pca-connector-ad-openapi-api-evangelist-ratings.yml
    type: API Evangelist Ratings
properties:
  - url: >-
      https://docs.aws.amazon.com/privateca/latest/userguide/connector-for-ad.html
    type: Documentation
  - url: properties/pca-connector-ad-openapi-original.yml
    type: OpenAPI
description: >-
  The Amazon Web Services Private CA Connector for Active Directory establishes
  a connection between Amazon Web Services Private CA and Active Directory,
  allowing you to generate security certificates for Active Directory that are
  signed by your own private CA. For further details, visit the Amazon Web
  Services Private CA Connector for Active Directory documentation.

---