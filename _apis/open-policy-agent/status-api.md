---
aid: open-policy-agent:status-api
name: Status API
tags:
  - Status
humanURL: https://www.openpolicyagent.org/docs/latest/rest-api/#status-api
properties:
  - url: https://www.openpolicyagent.org/docs/latest/rest-api/#status-api
    type: Documentation
  - url: properties/status-api.yml
    type: OpenAPI
  - url: status/bruno.json
    type: BrunoCollection
  - url: >-
      https://api-evangelist.postman.co/workspace/Open-Policy-Agent~bcc99d6c-728a-4d86-83fd-b6495f5e8fb8/collection/35240-b4943ca3-d651-4c68-a003-e6ca7feace03
    type: PostmanCollection
description: >-
  API for accessing OPA (Open Policy Agent) status information via a pull-based
  mechanism.

---