---
aid: open-policy-agent:config-api
name: Config API
tags:
  - Configurations
humanURL: https://www.openpolicyagent.org/docs/latest/rest-api/#config-api
properties:
  - url: https://www.openpolicyagent.org/docs/latest/rest-api/#config-api
    type: Documentation
  - url: properties/config-api.yml
    type: OpenAPI
  - url: config/bruno.json
    type: BrunoCollection
  - url: >-
      https://api-evangelist.postman.co/workspace/Open-Policy-Agent~bcc99d6c-728a-4d86-83fd-b6495f5e8fb8/collection/35240-979cbaf7-8515-4fd2-8035-134685590967
    type: PostmanCollection
description: >-
  API for retrieving OPA's active configuration, including discovered
  configurations.

---