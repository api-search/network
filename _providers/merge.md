---
api_count: 10
apis:
- description: Merge is the leading provider of customer-facing integrations for frontier LLMs, Fortune 500 organizations, and thousands of other B2B SaaS companies.
  name: Merge
  slug: merge
- description: Merge HRIS API provides a unified interface to integrate every HR, payroll, and SCIM directory system with one API. It normalizes data across 80+ HR platforms including Workday, BambooHR, Gusto, Rippl
  name: Merge HRIS API
  slug: hris-api
- description: Merge ATS API enables connection to every applicant tracking system with one API. It provides standardized data schemas for candidates, applications, interviews, and job postings across 50+ recruiting
  name: Merge ATS API
  slug: ats-api
- description: Merge Accounting API provides a unified API for accounting integrations, enabling read and write access to financial data across major accounting systems including QuickBooks Online, Xero, NetSuite, a
  name: Merge Accounting API
  slug: accounting-api
- description: Merge Ticketing API provides unified access to 30+ ticketing and project management systems including Jira, Asana, Linear, Zendesk, Freshdesk, GitHub Issues, and ServiceNow. It normalizes ticketing da
  name: Merge Ticketing API
  slug: ticketing-api
- description: Merge CRM API provides unified access to 20+ CRM platforms including Salesforce, HubSpot, Pipedrive, and Zoho CRM. It offers read and write capabilities for standardized CRM data objects such as Accou
  name: Merge CRM API
  slug: crm-api
- description: Merge File Storage API provides unified access to file storage platforms including Box, Dropbox, Google Drive, OneDrive, and SharePoint. It normalizes access to Drives, Files, Folders, Groups, and Use
  name: Merge File Storage API
  slug: file-storage-api
- description: Merge Knowledge Base API provides unified access to knowledge base platforms including Confluence and Notion. It normalizes access to Articles, Attachments, Containers, Groups, and Users, with ACL man
  name: Merge Knowledge Base API
  slug: knowledge-base-api
- description: Merge Chat Unified API provides real-time, normalized access to chat and messaging platforms including Microsoft Teams, with Slack on the roadmap. It normalizes five core object types including Messag
  name: Merge Chat API
  slug: chat-api
- description: 'Merge Agent Handler enables AI agents to connect with thousands of pre-built tools for taking real-time actions. It manages authentication, rate limits, error handling, and provides security controls '
  name: Merge Agent Handler
  slug: agent-handler
capabilities:
- description: Unified workflow combining HRIS and ATS APIs for end-to-end talent management, from candidate sourcing through onboarding and employee lifecycle. Used by HR teams, recruiters, and people operations.
  name: Merge Talent Management
  slug: talent-management
common:
- title: ''
  type: Website
  url: https://www.merge.dev/
- title: ''
  type: Security
  url: https://www.merge.dev/security
- title: ''
  type: Customers
  url: https://www.merge.dev/case-studies
- title: ''
  type: Blog
  url: https://www.merge.dev/blog
- title: ''
  type: eBooks
  url: https://www.merge.dev/resources?content-type=Ebooks
- title: ''
  type: Support
  url: https://help.merge.dev/
- title: ''
  type: ChangeLog
  url: https://www.merge.dev/changelog
- title: ''
  type: Webinars
  url: https://www.merge.dev/resources?content-type=Webinars
- title: ''
  type: Pricing
  url: https://www.merge.dev/pricing
- title: ''
  type: Documentation
  url: https://docs.merge.dev/?_gl=1*1llee1i*_gcl_au*MTY4MjQ4NzcxMy4xNzUyNjE4Mjgw*_ga*MTY0NjYzNTAyLjE3NTI2MTgyODA.*_ga_S6X9VBDBJN*czE3NTI2MTgyODAkbzEkZzEkdDE3NTI2MTg0NTMkajI3JGwwJGgw
- title: ''
  type: TermsOfService
  url: https://www.merge.dev/legal/terms
- title: ''
  type: PrivacyPolicy
  url: https://www.merge.dev/legal/privacy-policy
- title: ''
  type: GDPR
  url: https://www.merge.dev/eu
- title: ''
  type: Login
  url: https://app.merge.dev/login?_gl=1*6bfh40*_gcl_au*MTY4MjQ4NzcxMy4xNzUyNjE4Mjgw*_ga*MTY0NjYzNTAyLjE3NTI2MTgyODA.*_ga_S6X9VBDBJN*czE3NTI2MTgyODAkbzEkZzEkdDE3NTI2MTg2MjEkajMyJGwwJGgw
- title: ''
  type: SignUp
  url: https://app.merge.dev/signup
- title: ''
  type: StatusPage
  url: https://status.merge.dev/
- title: ''
  type: Trust
  url: https://trust.merge.dev/
- title: ''
  type: GitHubOrg
  url: https://github.com/merge-api
- title: ''
  type: PythonSDK
  url: https://github.com/merge-api/merge-python-client
- title: ''
  type: NodeSDK
  url: https://github.com/merge-api/merge-sdk-typescript
- title: ''
  type: GoSDK
  url: https://github.com/merge-api/merge-go-client
- title: ''
  type: JavaSDK
  url: https://github.com/merge-api/merge-java-client
- title: ''
  type: SDKs
  url: https://docs.merge.dev/sdk/
- title: ''
  type: PostmanWorkspace
  url: https://www.postman.com/mergeapi/workspace/merge-public-workspace/overview
- title: ''
  type: IntegrationDirectory
  url: https://www.merge.dev/integrations
- title: ''
  type: DeveloperTools
  url: https://www.merge.dev/features/developer-tools
- title: ''
  type: Observability
  url: https://www.merge.dev/features/integration-observability
- title: ''
  type: IntegrationManagement
  url: https://www.merge.dev/features/integrations-management
- title: ''
  type: Authentication
  url: https://docs.merge.dev/basics/authentication/
- title: ''
  type: GettingStarted
  url: https://docs.merge.dev/guides/testing-via-postman/
- title: ''
  type: GettingStarted
  url: https://docs.merge.dev/get-started/unified-api/
- title: ''
  type: SpectralRules
  url: rules/merge-spectral-rules.yml
- title: ''
  type: Vocabulary
  url: vocabulary/merge-vocabulary.yaml
- title: Talent Management Workflow
  type: NaftikoCapability
  url: capabilities/talent-management.yaml
created: '2025-06-05'
description: Merge is a Unified API that offers developers the ability to embed customer-facing API integrations in their products without the need to configure on a per-integration basis. With this integration, you can build and manage the integration with Recruitee in Merge's developer portal.
features:
- name: Observability
- name: Unified API
- name: Linked Accounts
- name: Self-Service Guides
- name: Data Syncs
- name: Drive Revenue
- name: HRIS
- name: ATS
- name: Accounting
- name: Ticketing
- name: CRM
- name: File Storage
- name: Authentication
- name: Pagination
- name: Syncing Data
- name: Sync Frequency
- name: Rate Limits
- name: Webhooks
- name: Versioning
- name: Async Operations
- name: Idepotency
- name: Model Context Protocol (MCP)
- name: Embedded Link
- name: Magic Link
- name: Merge Writes
- name: Troubleshooting Writes
- name: Integration Metadata
- name: Remote Data
- name: Passthrough Requests
- name: Remote Field Classes
- name: Field Mapping
- name: Custom Objects
- name: Compliance
- name: SSO
- name: Signin With Google
- name: Knowledge Base
- name: Chat
- name: Agent Handler
- name: Sandbox Accounts
- name: File Picker
image: /assets/icons/merge.png
integrations:
- description: HR platform integration via HRIS unified API.
  name: Workday
- description: HR and payroll integration via HRIS unified API.
  name: BambooHR
- description: ATS integration for recruiting workflows.
  name: Greenhouse
- description: ATS integration for candidate management.
  name: Lever
- description: Accounting integration for financial data.
  name: QuickBooks Online
- description: Accounting integration for invoicing and payments.
  name: Xero
- description: Ticketing integration for project management.
  name: Jira
- description: CRM integration for sales data.
  name: Salesforce
- description: CRM integration for marketing and sales.
  name: HubSpot
- description: File storage integration for document management.
  name: Box
- description: File storage integration for cloud files.
  name: Google Drive
- description: Chat integration for messaging (roadmap).
  name: Slack
- description: Chat integration for enterprise messaging.
  name: Microsoft Teams
- description: Knowledge base integration for enterprise content.
  name: Confluence
jsonld:
- class_count: 10
  name: Merge Accounting Api Context
  property_count: 24
  slug: merge-accounting-api-context
- class_count: 11
  name: Merge Ats Api Context
  property_count: 33
  slug: merge-ats-api-context
- class_count: 7
  name: Merge Crm Api Context
  property_count: 22
  slug: merge-crm-api-context
- class_count: 5
  name: Merge File Storage Api Context
  property_count: 17
  slug: merge-file-storage-api-context
- class_count: 13
  name: Merge Hris Api Context
  property_count: 39
  slug: merge-hris-api-context
- class_count: 4
  name: Merge Ticketing Api Context
  property_count: 20
  slug: merge-ticketing-api-context
layout: provider
modified: '2026-04-18'
name: Merge
rules:
- name: Merge API Rules
  rule_count: 37
  severity_counts:
    error: 15
    hint: 0
    info: 7
    warn: 15
  slug: merge-spectral-rules
skills: []
slug: merge
solutions:
- description: Single API for 80+ HR and payroll platforms.
  name: Unified HRIS
- description: Single API for 50+ applicant tracking systems.
  name: Unified ATS
- description: Single API for major accounting platforms.
  name: Unified Accounting
- description: Single API for 30+ ticketing and project management systems.
  name: Unified Ticketing
- description: Single API for 20+ CRM platforms.
  name: Unified CRM
- description: Single API for file storage platforms with File Picker.
  name: Unified File Storage
- description: AI agent tool orchestration for thousands of pre-built integrations.
  name: Agent Handler
tags:
- Integrations
- Platform
- Unified API
url: https://raw.githubusercontent.com/api-evangelist/merge/refs/heads/main/apis.yml
use_cases:
- name: Power AI features
- name: Auto-provision
- name: Knowledge base
- name: Financial analysis
- name: Candidate sourcing
- name: Project analysis
- name: Source leads
- name: Reconcile vendor payments
- name: Reconcile customer payments
- name: Enterprise search
- name: Customer support monitoring
- name: Document management
---

